  ...
  return (
    <div className='sticky z-20 top-0 w-full flex flex-row justify-between items-center p-2 bg-white'>
        ...
        <div className={`${ open ? 'h-[152px]' : 'h-0'} w-full z-20 transition-all fixed flex gap-2 overflow-hidden flex-col right-0 top-[3.25rem] bg-white shadow-xl`}>
            ...
            <button className='my-1 underline' onClick={() => {navigate('/quiz'); setOpen(!open)}}>QUIZZES</button>
            <button className='my-1 underline'>CATEGORY</button>
            <button className='my-1 underline'>MULTIPLAYER</button>
            <button className='my-1 underline'>CONTACT</button>
        </div>
    </div>
  )
}
